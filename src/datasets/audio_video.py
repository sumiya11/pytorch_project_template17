...
class AudioDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        speaker1, speaker2 = self.files['s1'][idx].split(".")[0].split("_")
        format = 'wav'
        # print(self.base / self.files['s1'][idx])
        signal1, sr1 = torchaudio.load(
            self.base / 's1' / self.files['s1'][idx],
            format=format
        )
        signal2, sr2 = torchaudio.load(
            self.base / 's2' / self.files['s2'][idx],
            format=format
        )
        mix, sr3 = torchaudio.load(
            self.base / 'mix' / self.files['mix'][idx],
            format=format
        )
        assert sr1 == sr2 == sr3
        sr = sr1
        ...
        sample = {'speaker1' : speaker1, 'speaker2' : speaker2, 'signal1': signal1, 'signal2': signal2, 'mixed': mix, 'sr' : sr}
