...
class Synthesizer(nn.Module):
    def __init__(
            ...
            pace=1.0,
            speaker=0,
            from_pretrained_hifi="saved/model_best.pth",
        ...
        pretrained_path = str(from_pretrained_hifi)
        print(f"Loading HiFi GAN model weights from: {pretrained_path} ...")
        
        checkpoint = torch.load(pretrained_path)
        self.vocoder.load_state_dict(checkpoint["state_dict"])
    ...
    def losses(self, batch, optimizer=None):
        losses = self.vocoder.losses(batch, optimizer)
        return losses
